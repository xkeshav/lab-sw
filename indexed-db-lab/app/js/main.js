...
var idbApp = (function() {
  ...
  var dbPromise = idb.open('couches-n-things', 5, function(upgradeDb) {
    switch (upgradeDb.oldVersion) {
      ...
      case 4:
        console.log('Creating a order store');
  ...
  function getByDesc() {
    ...
    dbPromise.then(function(db) {
      ...
      s += '</p>';
      return cursor.continue().then(showDesc);
  ...
  function addOrders() {
    dbPromise.then(function(db) {
      ...
      items.forEach(function(item) {
        console.log('Adding item in orders: ', item);
        store.add(item);
      });
      return tx.complete;
    }).then(function() {
  ...
  function updateProductsStore(products) {
    dbPromise.then(function(db) {
      ...
      // TODO 5.7 - update the items in the 'products' object storedbPromise.then(function(db) {
      var tx = db.transaction('products', 'readwrite');
      var store = tx.objectStore('products');
